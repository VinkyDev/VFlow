-- =========================================================
-- SECTION 1: 模块入口
-- ItemLayout — 物品组图标创建、冷却、布局（追加 + 单独分组）
-- =========================================================

local VFlow = _G.VFlow
if not VFlow then return end

if not VFlow.ModuleControlConstants.ITEMS_ENABLED then return end
...
-- 从 ItemGroups 获取共享状态与内部函数
local MODULE_KEY = IG._MODULE_KEY
local _standaloneFrameLists = IG._standaloneFrameLists
local _appendFrameLists = IG._appendFrameLists

local GetConfigForGroupId = IG.getConfigForGroupId
local ShouldStandaloneExtract = IG.shouldStandaloneExtract
local ShouldAppendToViewer = IG.shouldAppendToViewer
local ShouldShowItemGroup = IG._shouldShowItemGroup
local EnsureGroupContainer = IG._ensureGroupContainer
local ApplyContainerAnchor = IG._applyContainerAnchor
local ViewerCacheKey = IG._viewerCacheKey
local ResolveManualItemForTracking = IG._resolveManualItemForTracking
local ReleaseGroupContainer = IG._releaseGroupContainer
...
-- 前向引用：OnCooldownDone 里需同步灰度/swipe
...
local function LayoutStandaloneIconGrid(container, cfg, groupId, icons)
    ...
    for rowIdx, rowIcons in ipairs(rows) do
        ...
        for colIdx, button in ipairs(rowIcons) do
            ...
            UpdateItemStackDisplay(button, FD(button).entry, cfg)
        end
...
local function SyncStandaloneFrameList(container, groupId, cfg, entries)
    ...
    for i = 1, #entries do
        ...
        FD(frame).itemGroupId = groupId
        FD(frame).entry = entry
...
local function RefreshAllStandaloneLayouts()
    local db = VFlow.getDBIfReady(MODULE_KEY)
    if not db then
        return
    end
...
local function AppendCellsForGroup(viewer, groupId, cfg)
    ...
    local cells = {}
    -- MergeSkillRowsWithAppend 前会先完成 SyncAllAppendForViewer，这里直接复用同步后的 FD(frame).entry，
    -- 避免同一轮 Viewer 刷新里重复 BuildAppendEntries / BuildTrackedEntries。
    for i = 1, #list do
        local frame = list[i]
        local entry = frame and FD(frame).entry
...
local function RefreshAllAppendCooldowns()
    for _, groups in pairs(_appendFrameLists) do
        for groupId, list in pairs(groups) do
            local icfg = GetConfigForGroupId(groupId)
            for _, f in ipairs(list) do
                if f and FD(f).entry and ApplyEntryCooldown then
                    ApplyEntryCooldown(f, FD(f).entry)
                    UpdateAppendItemStackDisplay(f, FD(f).entry)
                    ApplyItemZeroCountPresentation(f, FD(f).entry, icfg)
                end
            end
        end
    end
...
local function RefreshStandaloneCooldownsOnly()
    for groupId, list in pairs(_standaloneFrameLists) do
        ...
        for _, frame in ipairs(list) do
            if frame and FD(frame).entry and ApplyEntryCooldown then
                ApplyEntryCooldown(frame, FD(frame).entry)
                UpdateItemStackDisplay(frame, FD(frame).entry, cfg)
                ApplyItemZeroCountPresentation(frame, FD(frame).entry, cfg)
            end
        end
    end
end
...
-- =========================================================
-- SECTION 10: 注入公开 API 到 VFlow.ItemGroups
-- =========================================================

IG.refreshStandaloneLayouts = RefreshAllStandaloneLayouts
IG.layoutItemGroups = RefreshAllStandaloneLayouts
IG.viewerHasAppendEntries = ViewerHasAppendEntries
IG.mergeSkillRowsWithAppend = MergeSkillRowsWithAppend
IG.buildViewerRowCells = MergeSkillRowsWithAppend
IG.syncAppendFramesForViewer = SyncAllAppendForViewer
IG.refreshAppendFrameStack = UpdateAppendItemStackDisplay
IG.refreshAllAppendCooldowns = RefreshAllAppendCooldowns
IG.refreshStandaloneCooldownsOnly = RefreshStandaloneCooldownsOnly

-- 内部回调：供 ItemGroups Store.watch 调用
IG._pruneOrphanRuntime = PruneOrphanItemGroupRuntime

IG.forEachAppendFrame = function(viewerName, callback)
...
IG.forEachStandaloneIcon = function(callback)
    if type(callback) ~= "function" then
        return
    end
