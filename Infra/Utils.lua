...
-- 12.0 起 SetCooldown 不可传含 secret 值的参数，用 Duration 对象绕过
function Utils.setCooldownFromStartAndDuration(cd, hostFrame, startTime, durationSeconds)
    ...
    local FD = VFlow.FD
    local fd = FD(hostFrame)
    if not fd.cooldownDurObj then
        fd.cooldownDurObj = C_DurationUtil.CreateDuration()
    end
    local ok = pcall(function()
        fd.cooldownDurObj:SetTimeFromStart(startTime, durationSeconds)
        cd:SetCooldownFromDurationObject(fd.cooldownDurObj)
    end)
    return ok
end
